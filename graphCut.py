...
from custom_io import read_img
from custom_io import increment_status_message
from cv2 import imwrite

from graphEnums import GenDirection as GenDirection
from graphEnums import GenMethod as GenMethod

def main():
    path_in = sys.argv[1] if len(sys.argv) > 1 else quit()
    path_out = sys.argv[2] if len(sys.argv) > 2 else "./"
    pattern = read_img(path_in).astype(np.int32)
    h, w = pattern.shape[:2]
    ...
    target_h = int(sys.argv[6]) if len(sys.argv) > 2 else int(2*h)
    target_w = int(sys.argv[7]) if len(sys.argv) > 3 else int(2*w)

    image_out = graphCut( pattern, direction, patchFactor, mode, target_h, target_w)
    imwrite(path_out, image_out)

def graphCut( image_in, direction = GenDirection.HORIZONAL, patchFactor = 8, mode = GenMethod.SUBBLOCK_ROW, h_in = 0, w_in = 0):
